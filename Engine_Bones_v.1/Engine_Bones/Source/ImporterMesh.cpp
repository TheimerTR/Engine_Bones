...
void Importer::ImporterMesh::ImportMesh(Mesh* M_mesh, const aiMesh* aiMesh)
{
	M_mesh->num_vertex = aiMesh->mNumVertices;
	M_mesh->vertex = new float[M_mesh->num_vertex * 3];
	memcpy(M_mesh->vertex, aiMesh->mVertices, sizeof(float) * M_mesh->num_vertex * 3);
	LOG(LogTypeCase::L_CASUAL, "New mesh with %d vertices", M_mesh->num_vertex);

	if (aiMesh->HasFaces())
	{
		M_mesh->num_index = aiMesh->mNumFaces * 3;
		...
		for (uint d = 0; d < aiMesh->mNumFaces; ++d)
		{
			...
			{
				memcpy(&M_mesh->index[d * 3], aiMesh->mFaces[d].mIndices, 3 * sizeof(uint));
	...
	{
		M_mesh->num_normals = aiMesh->mNumVertices;
		M_mesh->normals = new float[M_mesh->num_normals * 3];
		memcpy(M_mesh->normals, aiMesh->mNormals, sizeof(float) * M_mesh->num_normals * 3);

		M_mesh->center_normal = new float[aiMesh->mNumFaces * 3];
		M_mesh->center = new float[aiMesh->mNumFaces * 3];
		M_mesh->faces = aiMesh->mNumFaces;
	...
	{
		M_mesh->num_Tex = aiMesh->mNumVertices;
		M_mesh->textures = new math::float2[aiMesh->mNumVertices];

		for (int j = 0; j < M_mesh->num_Tex; j++)
		{
			M_mesh->textures[j].x = aiMesh->mTextureCoords[UV_Index][j].x;
			M_mesh->textures[j].y = aiMesh->mTextureCoords[UV_Index][j].y;
		}
	}
...
{
	uint ranges[4] = { M_mesh->num_vertex, M_mesh->num_index, M_mesh->num_Tex, M_mesh->num_normals };
	uint size = sizeof(ranges) + sizeof(uint) * M_mesh->num_index + sizeof(uint) * M_mesh->num_vertex * 3 + sizeof(uint) * M_mesh->num_normals * 3 + sizeof(uint) * M_mesh->num_Tex * 2;
	...
	*fileBuffer = new char[size]; // Allocate
	...
	// Store indices
	bytes = sizeof(uint) * M_mesh->num_index;
	memcpy(cursor, M_mesh->index, bytes);
	cursor += bytes;

	// Store vertex
	bytes = sizeof(uint) * M_mesh->num_vertex * 3;
	memcpy(cursor, M_mesh->vertex, bytes);
	cursor += bytes;

	// Store normals
	if(M_mesh->num_normals > 0)
	{
		bytes = sizeof(uint) * M_mesh->num_normals * 3;
		memcpy(cursor, M_mesh->normals, bytes);
		cursor += bytes;
	}
	...
	if(M_mesh->num_Tex > 0)
	{
		bytes = sizeof(uint) * M_mesh->num_Tex * 2;
		memcpy(cursor, M_mesh->textures, bytes);
		cursor += bytes;
	}
...
void Importer::ImporterMesh::Load(Mesh* M_mesh, const char* fileBuffer)
{
	...
	M_mesh->num_index = ranges[0];
	M_mesh->num_vertex = ranges[1];
	M_mesh->num_normals = ranges[2];
	M_mesh->num_Tex = ranges[3];

	// Load indices
	bytes = sizeof(uint) * M_mesh->num_index;
	M_mesh->index = new uint[M_mesh->num_index];
	memcpy(M_mesh->index, cursor, bytes);
	cursor += bytes;

	// Load vertex
	bytes = sizeof(uint) * M_mesh->num_vertex * 3;
	M_mesh->vertex = new float[M_mesh->num_vertex * 3];
	memcpy(M_mesh->vertex, cursor, bytes);
	cursor += bytes;
	...
	if (M_mesh->num_normals > 0)
	{
		bytes = sizeof(uint) * M_mesh->num_normals * 3;
		M_mesh->normals = new float[M_mesh->num_normals * 3];
		memcpy(M_mesh->normals, cursor, bytes);
		cursor += bytes;
	}
	...
	{
		bytes = sizeof(uint) * M_mesh->num_Tex * 2;
		M_mesh->textures = new math::float2[M_mesh->num_vertex];
		memcpy(M_mesh->textures, cursor, bytes);
		cursor += bytes;
	}
