...
bool ModuleFont::Init()
{
	...
	LOG(LogTypeCase::L_CASUAL, "Init Module Font");
...
Font* const ModuleFont::FontLoader(int size, const char* pathToFont)
{
	...
	if (error)
	{
		LOG(LogTypeCase::L_ERROR, "Free Type couldn't Init!");
	}
	...
	if (error)
	{
		LOG(LogTypeCase::L_ERROR, "Free Type couldn't New Face Font!");
	}
	...
	for (int i = 0; i < 128; i++)
	{
		...
		if (error)
		{
			LOG(LogTypeCase::L_ERROR, "Free Type couldn't load Char!");
			continue;
		}
